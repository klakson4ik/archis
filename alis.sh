...
function window_manager_i3wm(){
	pacman_install "xorg-server xorg-xinit i3-wm i3status i3lock dmenu"
}
...
function myfunc() {
	...
	cp -f xinitrc /mnt/home/$USER_NAME/.xinitrc
	cp -f vimrc /mnt/home/$USER_NAME/.vimrc
	cp -f config /mnt/home/$USER_HOME/.config/i3/
	cp -f config /mnt/home/$USER_HOME/.config/i3/
	curl -fLo /mnt/home/$USER_HOME/.vim/autoload/plug.vim --create-dirs \
...
function main() {
    ALL_STEPS=("configuration_install" "sanitize_variables" "check_variables" "warning" "init" "facts" "check_facts" "prepare" "partition" "install" "configuration" "mkinitcpio_configuration" "kernels" "mkinitcpio" "network" "users" "myfunc" "bootloader" "desktop_environment" "packages" "systemd_units" "terminate" "end")
